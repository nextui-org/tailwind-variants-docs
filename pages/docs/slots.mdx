...
import { WindowResizer, SlotsVariantsExample } from '@components'
...
<Callout type="info">
  <div className="text-sm">
    **Tailwind Variants** will automatically select the correct variant for each slot.
  </div>
</Callout>
...
<Callout type="error">
  <div className="text-sm">
   In responsive variants, you must declare either the `initial` variant or `defaultVariants` before any breakpoints are applied. 
  </div>
</Callout> 


```js copy /base/ /name/ /role/ /base()/ /name()/ /role()/
import { tv } from "tailwind-variants";
